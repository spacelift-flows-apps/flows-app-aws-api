...
const revokeRestoreAccessBackupVault: AppBlock = {
  name: "Revoke Restore Access Backup Vault",
  description: `Revokes access to a restore access backup vault, removing the ability to restore from its recovery points and permanently deleting the vault.`,
  inputs: {
    default: {
      config: {
        region: {
      ...
      onEvent: async (input) => {
        ...
        const command = new RevokeRestoreAccessBackupVaultCommand(
          commandInput as any,
        );
  ...
  outputs: {
    default: {
      name: "Revoke Restore Access Backup Vault Result",
      description: "Result from RevokeRestoreAccessBackupVault operation",
...
export default revokeRestoreAccessBackupVault;
